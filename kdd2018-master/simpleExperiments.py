...
class CompositeExperiment:
  ...
  def make_experiments(stump, evaluators, learners, generators):
    print('make_experiment 101', 'stump:', stump, 'evaluators:', evaluators, 'learners:', learners, 'generators:', generators)
    experiments = []
  ...
  def make_running_processes(experiments, output_dir):
    ...
    print('changed dir name mcv.MOA_DIR',mcv.MOA_DIR)
    os.chdir(mcv.MOA_DIR)
    utilities.remove_folder(output_dir)
    print('print output directory', output_dir)
...
class Utils: 
  ...
  def file_to_dataframe(some_file):
    print('some_file in simpleExperiment line 144:',some_file)
